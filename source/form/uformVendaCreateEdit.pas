...
type
  TformVendaCreateEdit = class(TformBase)
    ...
    procedure lbe_item_precoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure lbe_item_precoChange(Sender: TObject);
    procedure ds_itensDataChange(Sender: TObject; Field: TField);
    procedure act_descricaoExecute(Sender: TObject);
  private
    ...
    procedure findItem(text: string);
    procedure findUser;
    procedure calcTotal;
...
procedure TformVendaCreateEdit.act_descricaoExecute(Sender: TObject);
var
  v_form: TformDescription;
begin
  TAuthService.Description:= 'DESCRIÇÃO DO PRODUTO';
  try
  ...
  try
    fdmt_itens.Edit;
    fdmt_itensDESCRICAO.Text:= TAuthService.Description;
    fdmt_itens.Post;
  except on e: exception do
    THelper.Mensagem(e.Message);
  end;
...
procedure TformVendaCreateEdit.ds_itensDataChange(Sender: TObject;
  Field: TField);
begin
  inherited;
  lbl_descricao.Caption:= fdmt_itensDESCRICAO.Text;
end;
...
begin
  ...
  begin
    ...
    begin
       ...
       TAuthService.Venda.Itens.Last.Descricao:= fdmt_itensDESCRICAO.AsString;
...
procedure TformVendaCreateEdit.findItem(text: string);
var
...
begin
  ...
  try
    ...
    v_form.Tag:= 1;
    v_form.lbe_itens_search.Text:= text;
    v_form.list(text);
    v_form.ShowModal;
  finally
    FreeAndNil(v_form);
...
begin
  case Key of
    13: begin
      //Vai para formulario de finalizar a venda
      ...
      //Se ixistir item selecionado limpa
      if Assigned(Item) then FreeAndNil(Item);
      //Busca o item pelo código de referencia
      ...
      //Busca o produto direto na tela
      findItem(Trim(TLabeledEdit(Sender).text));
    end;
    27: cleanItem;
    38: begin
      fdmt_itens.Prior;
    ...
    112: findItem(TLabeledEdit(Sender).text);
  end;
end;
procedure TformVendaCreateEdit.lbe_item_precoChange(Sender: TObject);
begin
  inherited;
  if not Assigned(Item) then Exit;
  lbe_item_subtotal.Text:= THelper.ExtendedToString(THelper.StringToExtended(lbe_item_preco.Text) * THelper.StringToExtended(lbe_item_qtde.Text));
end;

procedure TformVendaCreateEdit.lbe_item_precoKeyDown(Sender: TObject;
