...
uses
  ...
  Vcl.ComCtrls, System.DateUtils, ACBrBase, ACBrDFe, ACBrNFe;
type
  TformPrincipal = class(TformBase)
    ...
    procedure act_osExecute(Sender: TObject);
  private
...
procedure TformPrincipal.act_osExecute(Sender: TObject);
begin
  inherited;
  try
    formOrdemServicoList:= TformOrdemServicoList.Create(nil);
    formOrdemServicoList.ShowModal;
  finally
    FreeAndNil(formOrdemServicoList);
  end;
end;
