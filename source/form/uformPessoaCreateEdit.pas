...
type
  TformPessoaCreateEdit = class(TformBase)
    ...
    procedure lbl_ConsultaDocClick(Sender: TObject);
  private
...
implementation
uses
  AuthService, udmRepository, Helper, uformConsultaCPF, uformConsultaCNPJ;
{$R *.dfm}
...
procedure TformPessoaCreateEdit.lbl_ConsultaDocClick(Sender: TObject);
var
  frmCPF: TformConsultaCPF;
  frmCNPJ: TFormConsultaCNPJ;
  I: Integer;
begin
  inherited;
  if cbx_TipoPessoa.ItemIndex = 0 then
  begin
    try
      frmCPF:= TformConsultaCPF.Create(nil);
      frmCPF.hint:= Thelper.ReturnsInteger(mke_documento.Text);
      frmCPF.ShowModal();
      if frmCPF.tag = 1 then
      begin
        lbe_nome.Text := frmCPF.CPF.Nome;
        dtp_data_fundacao_nascimento.Date:= frmCPF.EditDtNasc.date;
        mke_documento.Text:= frmCPF.EditCPF.text;

        mm_observacao.lines.Clear;
        mm_observacao.lines.add('Situação do CPF: ' + frmCPF.CPF.Situacao);
        mm_observacao.lines.add('');
        mm_observacao.lines.add('Data de cadastro: ' + frmCPF.CPF.DataInscricao);
      end;
    finally
      FreeAndNil(frmCPF);
    end;
  ...
  begin
    try
      frmCNPJ:= TFormConsultaCNPJ.Create(nil);
      frmCNPJ.hint:= THelper.ReturnsInteger(mke_documento.Text);
      frmCNPJ.ShowModal();
      if frmCNPJ.tag = 1 then
      begin
        mke_documento.Text:= Thelper.ReturnsInteger(frmCNPJ.CNPJ.CNPJ);
        lbe_nome.Text:= frmCNPJ.CNPJ.Fantasia;
        lbe_razao_social.Text:= frmCNPJ.CNPJ.RazaoSocial;
        dtp_data_fundacao_nascimento.Date:= frmCNPJ.CNPJ.Abertura;
        mke_Cep.text:= Thelper.ReturnsInteger(frmCNPJ.CNPJ.CEP);
        mke_cep.OnExit(mke_cep);

        lbe_logradouro.text:= frmCNPJ.CNPJ.Endereco;
        lbe_numero.text:= frmCNPJ.CNPJ.Numero;
        lbe_complemento.text:= frmCNPJ.CNPJ.Complemento;
        lbe_bairro.text:= frmCNPJ.CNPJ.Bairro;
        cbx_uf.ItemIndex:= cbx_uf.Items.IndexOf(frmCNPJ.CNPJ.UF);
        lbe_nome_municipio.text:= frmCNPJ.CNPJ.Cidade;
        lbe_email.text:= frmCNPJ.CNPJ.EndEletronico;
        mke_fone.text:= Thelper.ReturnsInteger(frmCNPJ.CNPJ.Telefone);

        mm_observacao.lines.Clear;
        mm_observacao.lines.add('Situação da Empresa: ' + frmCNPJ.CNPJ.Situacao);
        mm_observacao.lines.add('');
        for I := 0 to frmCNPJ.CNPJ.CNAE2.Count - 1 do
          mm_observacao.lines.add(frmCNPJ.CNPJ.CNAE2[I]);
      end;
    finally
      FreeAndNil(frmCNPJ);
    end;
